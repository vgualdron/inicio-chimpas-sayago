<template>
	<div>
		<b-row>
			<b-col cols="12">
				<b-card header="Modulos" tag="article" class="m-3 mt-3">
					<b-row>
						<b-col cols="12" sm="6" lg="3" v-if="aplicaciones.length > 0" v-for="aplicacion in aplicaciones"
							:key="'apli_' + aplicacion.id">
							<b-card :no-body="true" footer-class="px-3 py-2">
								<b-card-body class="p-3 clearfix">
									...
									<div class="text-muted text-uppercase font-weight-bold font-xs">
										{{ aplicacion.descripcion }}</div>
								</b-card-body>
								<div slot="footer">
									<b-link class="font-weight-bold font-xs btn-block text-muted" target="_blank"
										:href="aplicacion.url">Abrir Modulo <i
		...
		<b-row>
			<b-col cols="12">
				<b-card header="Información" tag="article" class="m-3 mt-3">
...
export default {
	...
	async mounted() {
		...
		await this.getResolution('FER', 'FE');
		setTimeout(() => this.getResolution('PI', 'POS'), 20000);
	}
}
</script>
